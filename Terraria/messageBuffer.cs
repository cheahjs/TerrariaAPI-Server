...
namespace Terraria
{
	public class messageBuffer
	{
		...
		public void GetData(int start, int length)
		{
			...
			if (b != 38 && Netplay.serverSock[this.whoAmI].state == -1)
			{
				NetMessage.SendData(2, this.whoAmI, -1, Lang.mp[1], 0, 0f, 0f, 0f, 0);
				return;
			}
			if (Netplay.serverSock[this.whoAmI].state < 10 && b > 12 && b != 16 && b != 42 && b != 50 && b != 38 && b != 68)
			{
				...
				for (int num44 = num42; num44 < num42 + (int)num41; num44++)
				{
					for (int num45 = num43; num45 < num43 + (int)num41; num45++)
					{
						...
						if (Main.tile[num44, num45].active())
						{
							...
							else if (!flag7 || (int)Main.tile[num44, num45].type != type3)
							{
			...
			else if (b == 22)
			{
				...
				if (Main.item[(int)num50].owner != this.whoAmI)
				{
					return;
				}
			...
			else if (b == 26)
			{
				...
				if (this.whoAmI != (int)b24 && (!Main.player[(int)b24].hostile || !Main.player[this.whoAmI].hostile))
				{
					return;
				}
				num++;
			...
			else if (b == 55)
			{
				...
				num += 2;
				if ((int)b39 != this.whoAmI && !Main.pvpBuff[(int)b40])
				{
					return;
				}
			...
			else if (b == 65)
			{
				...
				num++;
				short num132 = (short)this.whoAmI;
				num += 2;
				Vector2 newPos;
